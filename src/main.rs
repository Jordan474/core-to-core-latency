    ...
    for b in &args.bench {
        match b {
            1 => {
                ...
                eprintln!();
                run_bench(&cores, &clock, &args, &*Box::new(bench::cas::Bench::new()));
            }
            2 => {
                eprintln!();
                eprintln!("2) Single-writer single-reader latency on two shared cache lines");
                eprintln!();
                run_bench(&cores, &clock, &args, &*Box::new(bench::read_write::Bench::new()));
            }
            3 => {
                ...
                eprintln!();
                run_bench(&cores, &clock, &args, &*Box::new(bench::msg_passing::Bench::new(args.num_iterations)));
            }
